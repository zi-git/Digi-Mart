  ...
  return (
    <>
      <div className="w-full bg-[#f7f9fb] flex flex-col justify-between items-center gap-10 sm:pt-0 pt-1 scroll-smooth">
       ...
       <div className="w-full px-4 md:px-10 py-6 max-w-screen-xl mx-auto space-y-12">
  {/* SECTION COMPONENT */}
  {[
    { title: "Today's Deals - 30% Off", start: 0, end: 4, discount: 0.3 },
    { title: "Top Picks - 50% Off", start: 4, end: 8, discount: 0.5 },
    { title: "Recommended for You - 10% Off", start: 8, end: 12, discount: 0.1 },
  ].map((section, i) => (
    <div key={i}>
      <h1 className="text-2xl md:text-3xl font-bold mb-4">{section.title}</h1>
      <div className="w-full flex overflow-x-auto gap-4 p-2 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-100">
        {productList.slice(section.start, section.end).map((product, index) => {
          const originalPrice = (product.price / (1 - section.discount)).toFixed(2);
